...
for i=1:length(params.corrInfo)
	switch upper(params.corrInfo{i})
		...
		case 'N', corrtext = [corrtext; ['n=' mynum2str(stats.N)]];
	end
end
...
if strcmpi(params.data1TreatmentMode,'Compare')
	data.baRefData = mean([data.maskedSet1,data.maskedSet2],2);
else
	data.baRefData = data.maskedSet1; % previous version was calaculated as RPC/mean of data.
end
switch upper(params.diffValueMode)
	case 'ABSOLUTE'
		data.differences = data.maskedSet2-data.maskedSet1;
	case 'RELATIVE'
		data.differences = (data.maskedSet2-data.maskedSet1) ./ data.baRefData;
	case 'PERCENT'
		data.differences = (data.maskedSet2-data.maskedSet1) ./ data.baRefData*100;
end
stats.differenceSTD = std(data.differences);
stats.differenceMean = mean(data.differences);
stats.differenceMedian = median(data.differences);
[~, stats.differenceMeanP] = ttest(data.differences,0);
...
stats.rpcPercent = 1.96*std(data.differences ./ data.baRefData)*100; % previous version was calaculated as RPC/mean of data.
stats.IQR = iqr(data.differences);
...
[~, stats.ksp] = kstest((data.differences-stats.differenceMean)/stats.differenceSTD);
stats.kurtosis = kurtosis(data.differences);
setat.skewness = skewness(data.differences,1);


%% Plot the BA plot
function PlotBA(baAH, data, stats, params)
